...
.controller('DocsController', function($route, $scope, $rootScope, $location, $window, $cookies, NG_PAGES, NG_NAVIGATION, NG_VERSION) {
  ...
  var OFFLINE_COOKIE_NAME = 'ng-offline',
      ...
      $scope.nav = function($event) {
          ...
          if (!$location.$$html5) {
              ...
              hashBang = $location.absUrl().replace(encodeURI($location.path()), '').replace($event.target.baseURI, '');
              window.location.href = $event.target.href.replace($event.target.baseURI, $event.target.baseURI + hashBang + '/');
          }
      };
